...
// A dark key shape like axui::ModePill.
...
PresetBar::PresetBar(PresetController& c) : ctl(c) {
    ...
    prev.setDescription("Loads the previous preset in this folder");
    next.setDescription("Loads the next preset in this folder");
    save.setDescription("Saves the preset; the light is on when it has changes");
    more.setDescription("Save As, New Folder, Rename, Duplicate, Delete, " + PresetController::revealLabel());
...
void PresetBar::refresh() {
    ...
    String folder;
    if (ref.valid) folder = ref.folder.isEmpty() && !ref.factory ? String(axpresets::kUnfiledName) : ref.folder;
...
PromptSheet::PromptSheet() {
    ...
    for (auto* c : std::initializer_list<Component*>{ &nameField, &numberField, &folderBox, &ok, &cancel }) addChildComponent(c);
    folderBox.setTitle("Folder");
...
void PromptSheet::show(const Spec& s, std::function<void(bool, const Result&)> cb) {
    ...
    if (spec.folderNewItem) { folderBox.addSeparator(); folderBox.addItem("New Folder" + kEllipsis, spec.folders.size() + 1); }
...
void PromptSheet::layout() {
    ...
    for (auto* c : std::initializer_list<Component*>{ &nameField, &numberField, &folderBox, &ok, &cancel })
...
void PromptSheet::paint(Graphics& g) {
    ...
    label(folderBox, "Folder");
    if (numberField.isVisible())
        axui::drawText(g, "Optional, 0 to 999", axui::font(Face::Regular, 12.0f), hex(axui::col::textDim),
...
struct PresetBrowser::FolderModel : public ListBoxModel {
    ...
    void paintListBoxItem(int row, Graphics& g, int w, int h, bool selected) override {
        ...
        if (f.factory) drawLock(g, 14.0f, cy - 5.5f, 8.0f, 10.0f, selected ? Colours::white : hex(axui::col::textDim));
        ...
        axui::drawText(g, axui::ellipsize(nf, f.name, (float) w - 30.0f - cw - 22.0f), nf,
                       selected ? Colours::white : (f.unfiled ? hex(axui::col::textDim) : hex(0xe0e5ec)), 30.0f,
    ...
    String getNameForRow(int row) override {
        ...
        if (row < 0 || row >= (int) fs.size()) return {};
        const auto& f = fs[(size_t) row];
        return f.name + (f.factory ? ", factory, read-only" : String()) + ", " + String(f.presets.size()) + " presets";
    }
    ...
    void listBoxItemClicked(int row, const MouseEvent& e) override {
        ...
        m.addItem(1, "New Folder" + kEllipsis);
        m.addItem(2, "Rename Folder" + kEllipsis, user);
        m.addItem(3, "Delete Folder" + kEllipsis, user);
        m.addSeparator();
...
struct PresetBrowser::PresetModel : public ListBoxModel {
    ...
    void paintListBoxItem(int row, Graphics& g, int w, int h, bool selected) override {
        ...
        for (auto& q : f->presets) anyNumber = anyNumber || q.number >= 0;
        float x = 14.0f;
        if (anyNumber) {
            if (p->number >= 0) {
                const auto nf = axui::font(Face::SemiBold, 12.0f);
                axui::drawText(g, String(p->number).paddedLeft('0', 3), nf, selected ? Colours::white.withAlpha(0.85f) : hex(axui::col::textDim),
    ...
    String getNameForRow(int row) override {
        auto* p = preset(row);
        if (p == nullptr) return {};
        return (p->number >= 0 ? String(p->number).paddedLeft('0', 3) + " " : String()) + p->name;
    }
...
void PresetBrowser::showRowMenu(int row) {
    ...
    m.addItem(2, info.factory ? "Duplicate to Unfiled" : "Duplicate");
...
void PresetBrowser::paint(Graphics& g) {
    ...
    axui::drawText(g, "FOLDERS", capF, hex(axui::col::textDim), c.getX() + 18.0f, axui::baselineCentred(Face::NarrowBold, 11.0f, headerCy));
    const auto& fs = ctl.library.folders();
    ...
    if (folderIndex >= 0 && folderIndex < (int) fs.size()) {
        ...
        const String sub = f.factory ? "Factory, read-only" : (f.unfiled ? "Not in a folder" : String(f.presets.size()) + (f.presets.size() == 1 ? " preset" : " presets"));
        ...
        if (f.presets.empty()) {
            ...
            axui::drawText(g, "No presets in this folder.", ef, hex(axui::col::textDim), lb.getCentreX(),
...
void PresetController::step(int delta) {
    ...
    if (f == nullptr || f->presets.empty()) {
        ...
        if (bi >= 0 && bi < (int) fs.size() && !fs[(size_t) bi].presets.empty()) f = &fs[(size_t) bi];
        for (size_t i = 0; f == nullptr && i < fs.size(); ++i) if (!fs[i].presets.empty()) f = &fs[i];
    }
    if (f == nullptr) return;
    const auto ref = proc.currentPreset();
    ...
    int idx = -1;
    for (int i = 0; i < n; ++i)
        if (ref.valid && ref.factory == f->factory && ref.folder == f->presets[(size_t) i].folder && ref.fileName == f->presets[(size_t) i].fileName) idx = i;
    idx = idx < 0 ? (delta > 0 ? 0 : n - 1) : ((idx + delta) % n + n) % n;
    const auto info = f->presets[(size_t) idx];
    load(info);
}

void PresetController::save() {
    const auto ref = proc.currentPreset();
    if (ref.valid && !ref.factory) {
        ...
        if (file.existsAsFile()) {
            ...
            browser.setNotice("Saved " + quoted(ref.name) + ".", false);
            if (browser.isVisible()) browser.rebuild(true);
            bar.refresh();
            return;
        }
...
void PresetController::showSaveAs(const SaveAsState& s) {
    library.rescan();
    StringArray names, folderKeys;
    int index = 0;
    for (auto& f : library.folders()) {
        if (f.factory) continue;
        names.add(f.name);
        folderKeys.add(f.unfiled ? String() : f.name);
        if ((f.unfiled && s.folder.isEmpty()) || (!f.unfiled && f.name == s.folder)) index = names.size() - 1;
    }
    ...
    spec.numberValue = s.number;
    spec.folders = names;
    spec.folderIndex = index;
    spec.folderNewItem = true;
    spec.okText = "Save";
    spec.error = s.error;
    sheet.show(spec, [this, folderKeys](bool ok, const PromptSheet::Result& r) {
        if (!ok) return;
        SaveAsState next;
        next.name = r.name.trim();
        next.number = r.number.trim();
        next.folder = r.folderIndex >= 0 && r.folderIndex < folderKeys.size() ? folderKeys[r.folderIndex] : String();
        if (r.newFolderChosen) {
            showNewFolder(uniqueFolderName(), {}, [this, next](const String& created) {
                auto n = next;
                n.folder = created;
                showSaveAs(n);
            });
        ...
        if (file.exists()) {
            PromptSheet::Spec c;
            c.title = "Replace " + quoted(next.name) + "?";
            c.message = "A preset with this name is in " + quoted(next.folder.isEmpty() ? String(axpresets::kUnfiledName) : next.folder)
                        + ". If you replace it, the file is overwritten.";
            c.okText = "Replace";
            c.destructive = true;
            sheet.show(c, [this, next](bool yes, const PromptSheet::Result&) {
                if (yes) writeSaveAs(next);
...
void PresetController::writeSaveAs(const SaveAsState& s) {
    const int number = s.number.isEmpty() ? -1 : jlimit(0, 999, s.number.getIntValue());
    ...
    ref.number = number;
    proc.setCurrentPreset(ref);
    browser.setNotice("Saved " + quoted(s.name) + ".", false);
...
String PresetController::uniqueFolderName() const {
    ...
    for (int i = 2; library.dirForUserFolder(name).exists() && i < 1000; ++i) name = "New Folder " + String(i);
    return name;
}

void PresetController::newFolder(std::function<void(const String&)> then) {
    showNewFolder(uniqueFolderName(), {}, std::move(then));
}

void PresetController::showNewFolder(const String& value, const String& error, std::function<void(const String&)> then) {
    PromptSheet::Spec spec;
    spec.title = "New Folder";
    spec.message = "The folder is made in " + library.userRoot().getFullPathName() + ".";
    spec.nameLabel = "Name";
    ...
    sheet.show(spec, [this, then](bool ok, const PromptSheet::Result& r) {
        if (!ok) return;
        const auto res = library.createFolder(r.name);
        if (res.failed()) { showNewFolder(r.name, res.getErrorMessage(), then); return; }
...
void PresetController::renamePreset(const axpresets::PresetInfo& info) {
    if (!info.factory) showRenamePreset(info, info.name, {});
}

void PresetController::showRenamePreset(const axpresets::PresetInfo& info, const String& value, const String& error) {
    PromptSheet::Spec spec;
    ...
    spec.okText = "Rename";
    spec.error = error;
    sheet.show(spec, [this, info](bool ok, const PromptSheet::Result& r) {
        if (!ok || r.name.trim() == info.name) return;
        const auto ref = proc.currentPreset();
        const bool wasCurrent = ref.valid && !ref.factory && ref.folder == info.folder && ref.fileName == info.fileName;
        const auto res = library.renamePreset(info, r.name, AX330GChainProcessor::pluginVersionString());
        if (res.failed()) { showRenamePreset(info, r.name, res.getErrorMessage()); return; }
        if (wasCurrent) {
            auto n = ref;
            n.name = r.name.trim();
...
void PresetController::duplicatePreset(const axpresets::PresetInfo& info) {
    String newFile;
    const auto res = library.duplicatePreset(info, AX330GChainProcessor::pluginVersionString(), &newFile);
    if (res.failed()) { showError("Cannot duplicate the preset", res.getErrorMessage()); return; }
    browser.setNotice("Made " + quoted(newFile.upToLastOccurrenceOf(".", false, false)) + (info.factory ? String(" in ") + axpresets::kUnfiledName : String()) + ".", false);
    if (browser.isVisible()) {
        browser.rebuild(true);
        if (info.factory) {
            const auto& fs = library.folders();
            for (size_t i = 0; i < fs.size(); ++i) if (fs[i].unfiled) browser.selectFolder((int) i);
        }
    }
}
...
void PresetController::renameFolder(const axpresets::FolderInfo& f) {
    if (!f.factory && !f.unfiled) showRenameFolder(f.name, f.name, {});
}

void PresetController::showRenameFolder(const String& oldName, const String& value, const String& error) {
    PromptSheet::Spec spec;
    spec.title = "Rename Folder";
    spec.nameLabel = "Name";
    spec.nameValue = value;
    spec.okText = "Rename";
    spec.error = error;
    sheet.show(spec, [this, oldName](bool ok, const PromptSheet::Result& r) {
        if (!ok || r.name.trim() == oldName) return;
        const auto res = library.renameFolder(oldName, r.name);
        if (res.failed()) { showRenameFolder(oldName, r.name, res.getErrorMessage()); return; }
        const String newName = axpresets::Library::legalName(r.name);
        auto ref = proc.currentPreset();
        if (ref.valid && !ref.factory && ref.folder == oldName) { ref.folder = newName; proc.setCurrentPreset(ref, proc.isPresetModified()); }
        if (browser.isVisible()) {
            browser.rebuild(true);
            const auto& fs = library.folders();
...
void PresetController::deleteFolder(const axpresets::FolderInfo& f) {
    ...
    PromptSheet::Spec spec;
    spec.title = "Move the folder " + quoted(f.name) + " to the " + trashName() + "?";
    const int n = (int) f.presets.size();
    spec.message = (n == 0 ? String("The folder is empty. ") : "It holds " + String(n) + (n == 1 ? " preset. " : " presets. "))
    ...
    sheet.show(spec, [self, name](bool ok, const PromptSheet::Result&) {
        ...
        if (res.failed()) { self->showError("Cannot delete the folder", res.getErrorMessage()); return; }
        const auto ref = self->proc.currentPreset();
...
void PresetController::showMenu() {
    ...
    m.addItem(2, "New Folder" + kEllipsis);
    m.addSeparator();
    m.addItem(3, "Rename Preset" + kEllipsis, userPreset);
    m.addItem(4, info != nullptr && info->factory ? "Duplicate to Unfiled" : "Duplicate Preset", info != nullptr);
    m.addItem(5, "Delete Preset" + kEllipsis, userPreset);
    m.addSeparator();
    m.addItem(6, userFolder ? "Rename Folder " + quoted(folder->name) + kEllipsis : "Rename Folder" + kEllipsis, userFolder);
    m.addItem(7, userFolder ? "Delete Folder " + quoted(folder->name) + kEllipsis : "Delete Folder" + kEllipsis, userFolder);
    m.addSeparator();
